...
| **Calendar** (`/calendar`) | When Canary can text: founder free/busy from Google Calendar (or a private iCal feed). Meetings hold iMessage alerts until they end. Ledger cash stays on `/ledger`. |
| **Needs Review** (`/needs-review`) | Every uncorroborated transaction with the disagreeing signals that put it there, and an assign-category action |
| **Ask Canary** | ElevenLabs **Start a call** widget, bottom-right of every page (auto-mounted once a ticket exists). The Worker mints a short-lived signed URL so the API key never reaches the browser. `/ask` explains it. Same deterministic tools as iMessage. A short explainer stays in that corner when voice is not connected. |

| Ledger | When Canary can text | Needs Review |
| --- | --- | --- |
...
- **Google Calendar (real OAuth, one founder account).** Connect once at `https://canary.bill-nguyentonhoang.workers.dev/oauth/google/start?secret=<WEBHOOK_SECRET>` (locally `http://localhost:8787/oauth/google/start?secret=<WEBHOOK_SECRET>`). `start` sends Google a `login_hint` from `FOUNDER_EMAIL` when set. The refresh token is stored AES-GCM-encrypted in D1 and tokens are refreshed with plain `fetch`. Canary reads live free/busy from the primary calendar for the notification policy and the calendar page's busy blocks, and can **book a 15-minute review** in the next free business-hours slot (`POST /api/incidents/:id/schedule-review`, or reply `SCHEDULE` in iMessage). `FOUNDER_EMAIL` is the expected Google account (example in `.dev.vars.example`: `bill.nguyentonhoang@gmail.com`); a different account is flagged on the callback page, not rejected, and that account is still used for free/busy. The OAuth app runs in Google's *Testing* mode (refresh tokens expire after 7 days — reconnect before a demo). Fallback when Google is not connected: one private iCal feed (`CALENDAR_ICS_URL`) read by a small RFC 5545 parser. With neither configured Canary has no availability signal and never defers an alert. Meeting titles are hidden unless `CALENDAR_SHOW_TITLES=1`. A broken Google read reports `source: "none"` and the founder looks free — a dead calendar must never hold a material alert.
...
| Tests | Vitest — 1,335 tests (4 skipped), offline |
