...
export function buildSystemPrompt({ derived, now }: PromptContext): string {
  ...
  return [
    ...
    "",
    "TOOLS",
    "- Call a tool for every figure. `get_health_summary` for cash, burn and runway. `get_incident` for what was flagged and why. `get_active_incidents` to list what is open. `get_vendor_spend` for one vendor's rate. `list_transactions` for recent charges (vendor, date range, or Needs Review). `simulate_cost_change` for any \"what if X were N% lower\" question. `get_evidence` for cited sources. `create_app_link` for a link.",
    "- `list_transactions` returns at most a handful of newest rows. Copy dates and amounts exactly. If `matched` is larger than `shown`, say so and offer a tighter filter. If `grain` is weekly_vendor_totals, say they are weekly totals, not individual card swipes.",
