...
export function Dashboard() {
  const { data, error, reload } = useDerived();
  ...
  const { burn, company, provenance } = data;

  return (
    <div className="space-y-6 sm:space-y-8">
      <header>
        <h1 className="text-xl font-semibold tracking-tight text-neutral-900 sm:text-2xl">Home</h1>
        <p className="mt-1 text-sm text-neutral-500">
          {data.weeks.length} weeks through {formatDateMedium(provenance.end_date)}
        </p>
      </header>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          caption={`${company.bank_name} balance · ${formatDateMedium(provenance.end_date)}`}
