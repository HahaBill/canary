...
generator  → 626 transactions over 52 Mon–Sun weeks, generated BACKWARD so opening + Σ cash = closing exactly
           │    plus a 26-week horizon of un-posted rows; the demo clock replays a bounded final slice OF HISTORY and never passes DEMO.END_DATE
...
- `npm test` — the full offline suite. Live OpenAI/Tavily/Rho tests opt in only when their keys or live flags are present.
